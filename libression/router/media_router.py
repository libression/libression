...
class FileEntry(pydantic.BaseModel):
    file_key: str
    file_entity_uuid: str  # Always filled!
...
class CopyRequest(pydantic.BaseModel):
    file_mappings: list[FileKeyMapping]
...
@router.post("/upload", response_model=list[FileEntry])
...
@router.post("/files_urls", response_model=PresignedUrlResponse)
...
@router.post("/copy", response_model=list[FileActionResponse])
...
@router.post("/delete", response_model=list[FileActionResponse])
