...
def test_malformated_file_integration_docker(specified_extension):
    ...
    docker_webdav_url = "https://webdav:443"  # nginx self-signed cert (docker address)
    filebytes = b"asdfasdf"
    base_64_image = base64.b64encode(filebytes).decode("utf-8")
    test_dir = f"test/path/{uuid.uuid4()}"
    test_tag = str(uuid.uuid4())
    ...
    upload_response = httpx.post(
        ...
        json={
            "files": [
                {
                    "filename": f"test.{specified_extension}",
                    "file_source": base_64_image,
                }
            ],
            "target_dir": test_dir,
        },
    ...
    # Copy file
    copied_file_key = f"{test_dir}/{test_dir}/copy.{specified_extension}"
    copy_response = httpx.post(
        f"{base_libression_url}/libression/v1/copy",
        json={
            "file_mappings": [
    ...
    # Move file
    moved_file_key = f"{test_dir}/{test_dir}/moved.{specified_extension}"
    move_response = httpx.post(
        f"{base_libression_url}/libression/v1/copy",
        json={
            "file_mappings": [
