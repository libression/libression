...
@pytest.mark.parametrize("pixels,expected_length", [
...
def test_phash_different_sizes(minimal_image, pixels, expected_length):
    ...
    hash_value = libression.thumbnail.phash.phash_from_thumbnail(minimal_image, pixels=pixels)
...
def test_phash_rotation_invariance(minimal_image):
    ...
    for angle in [90, 180, 270]:
        ...
        rotated_hash = libression.thumbnail.phash.phash_from_thumbnail(rotated_bytes.getvalue())
...
def test_phash_error_handling():
    ...
    # None input
    assert libression.thumbnail.phash.phash_from_thumbnail(None) == ''  # Changed from raises to return empty
    ...
    assert not libression.thumbnail.phash.compare_thumbnail_hashes(None, 'abc')  # Add None test
    assert not libression.thumbnail.phash.compare_thumbnail_hashes('abc', None)  # Add None test
